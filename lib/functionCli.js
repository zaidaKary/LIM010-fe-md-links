...
const functionCli = matrix => {
  ...
  if (matrix.length === 1) {
    output = (0, _mdLinks.mdLinks)(matrix[0]).then(res => (0, _stats.formatOutput)(res)).catch(e => e.message);
  }
  ...
  if (matrix.length === 2) {
    if (matrix[1] === '--stats' || matrix[1] === '--s') {
      ...
      .then(res => (0, _stats.stats)(res)).catch(e => e.message);
    } else if (matrix[1] === '--validate' || matrix[1] === '--v') {
      output = (0, _mdLinks.mdLinks)(matrix[0], {
        validate: true
      }).then(res => (0, _stats.formatOutput)(res)).catch(e => e.message);
    } else {
  ...
  if (matrix.length === 3) {
    if (matrix[1] === '--validate' && matrix[2] === '--stats' || matrix[1] === '--stats' && matrix[2] === '--validate') {
      output = (0, _mdLinks.mdLinks)(matrix[0], {
        validate: true
      }).then(res => (0, _stats.statValidate)(res)).catch(e => e.message);
    } else {
